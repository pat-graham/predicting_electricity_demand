...
acf(benchmark_model$residuals, main = "ACF of ARIMA Model Residuals Plot")
pacf(benchmark_model$residuals, main = "PACF of ARIMA Model Residuals Plot")
plot(benchmark_model$residuals, main = "visualisation of residuals for ARIMA")
...
ggplot() + 
  ...
  xlab("Year") + ylab("Aggregate Demand MhW") + ggtitle("ARIMA Performance on Test Set with Real Outcome") +
...
#Testing for heteroskedacisity in ARIMA
...
test_index2 = seq(from = round(0.8*nrow(aggregate_demand)) + 1, to = nrow(aggregate_demand), by = 1)
#set index to the previous index used so i dont have to change subsequent code

rolling_actual <- aggregate_demand[test_index2,2][c(20346:21017)] #actual values for the last month
test_roll <- rbind(aggregate_data_nn[test_index2,2][c(19674:20345)]) #set up inputs for rolling frame

rolling.fc <- vector("numeric", 672)

for(i in 1:672){
    predict <- model %>% predict(test_roll) #predict using inputs
  test_roll <- cbind(test_roll,predict) #bind prediction to the rolling frame 672->673
  test_roll <- rbind(test_roll[c(2:673)]) #chop off the first observation 673->672

  
  rolling.fc[i] <- predict #record prediction at that step
}

rolling.fc.scaled <- rolling.fc * attr(aggregate_data_nn$value, 'scaled:scale') + attr(aggregate_data_nn$value, 'scaled:center') #unscale the predictions
...
#calculate mean squared error - MAE would be better but too late....
rolling.mse <- mean((df$rolling_actual - df$rolling.fc.scaled)^2)
# Graphs to come

df %>% ggplot(aes(x=index)) + geom_line(aes(y=rolling.fc.scaled, group=1, colour="neural net")) + geom_line(aes(y=rolling_actual, group=1, colour="test")) + geom_line(aes(y = benchmark_forecast$mean,group=1, colour = "ARIMA")) + ylab("Aggregate Load") + theme(
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())

df %>% ggplot(aes(x=index)) + geom_line(aes(y=rolling.fc.scaled, group=1, colour="neural net")) + geom_line(aes(y=rolling_actual, group=1, colour="test"))  + ylab("Aggregate Load") + theme(
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
```
