...
test_roll <- rbind(test_data[1,])
test_roll_label <- rbind(test_label[1,])
...
rolling.fc <- rolling.fc * attr(aggregate_data_nn$value, 'scaled:scale') + attr(aggregate_data_nn$value, 'scaled:center')

df <- cbind(rolling.fc, test_label_unscaled[c(1:672)])
df <- data.frame(df) 
df <- df %>% mutate(index = rownames(df))

df %>% ggplot(aes(x=index)) + geom_line(aes(y=rolling.fc, group=1, colour="predictions")) + geom_line(aes(y=V2, group=1, colour="test")) + ylab("Aggregate Load") + theme(
