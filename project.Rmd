...
aggregate_demand_ts <- ts(aggregate_demand, frequency = 24) #turn into timeseries object
...
plot(acf(aggregate_demand_ts, lag.max = 24)) #ACF for one day
plot(acf(aggregate_demand_ts, lag.max = 7 * 24)) # ACF for one week 
plot(acf(aggregate_demand_ts, lag.max = 7 * 24 * 31)) # ACF for one month

# Decompose into seasonality and trend, plot the results
aggregate_components <- decompose(aggregate_demand) 
plot(aggregate_components) 

# Define training, validation and test
train_index = seq(from = 1, to = round(0.6*nrow(aggregate_demand)), by = 1)
validation_index = seq(from = round(0.6*nrow(aggregate_demand)) + 1, to = round(0.8*nrow(aggregate_demand)), by = 1)
test_index = seq(from = round(0.8*nrow(aggregate_demand)) + 1, to = nrow(aggregate_demand), by = 1)

# Fit ARIMA model and forecast
benchmark_model = auto.arima(aggregate_demand_ts[train_index,2], D = 1)
benchmark_forecast = forecast(benchmark_model, h = length(validation_index))

# Plot ARIMA forecast
ggplot() + 
  geom_line(aes(x = aggregate_demand[train_index,1], y = aggregate_demand[train_index,2], color = "train")) +
  geom_line(aes(x = aggregate_demand[validation_index,1], y = benchmark_forecast$mean, color = "forecast")) +
  geom_line(aes(x = aggregate_demand[validation_index,1], y = benchmark_forecast$upper[,2], color = "upper")) +
  geom_line(aes(x = aggregate_demand[validation_index,1], y = benchmark_forecast$lower[,2], color = "lower"))

ggplot() + geom_line(aes(x = time_span[train], y = aggregate_demand[train_index], color = "train"))+ geom_line(aes(x = time_span[test], y = aggregate_demand[test], color = "actual")) 

```

```{r Fitting a Neural Network Model}

# Redefine training, no need for validation as keras has this function in built
train_index = seq(from = 1, to =  round(0.8*nrow(aggregate_demand)), by = 1)

# Scale input data to be standardized
aggregate_data_nn = aggregate_data
...
generator_train <- timeseries_generator(data = as.matrix(train_data$value), targets = as.matrix(train_data$value), length = 672, batch_size = 10)
generator_test <- timeseries_generator(data = as.matrix(test_data$value), targets = as.matrix(test_data$value), length = 672, batch_size = 10)
