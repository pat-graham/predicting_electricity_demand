...
offset = 24*7
train = seq(2/3*length(aggregate_demand) - offset, 2/3*length(aggregate_demand), by = 1)
test = seq(2/3*length(aggregate_demand) + 1, 2/3*length(aggregate_demand) + offset, by = 1)

# Fit time series benchmark model
fit = auto.arima(aggregate_demand[train])

# Forecast on test set
demand_forecast = forecast(fit, h = length(test))

ggplot() + geom_line(aes(x = time_span[test], y = demand_forecast$mean, color = "forecast")) + geom_line(aes(x = time_span[train], y = aggregate_demand[train], color = "train"))
...
#ggplot() + geom_line(aes(x = time_span, y = aggregate_demand)) + xlab("Year") + ylab("Demand (GW)") + ggtitle("Aggregate US Electricity Demand") + theme(plot.title = element_text(hjust = 0.5))

#split = createTimeSlices(y = aggregate_demand, initialWindow = 10, horizon = 10)
